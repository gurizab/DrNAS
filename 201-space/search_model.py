...
from search_cells import NAS201SearchCell_PartialChannel as SearchCell
...
class TinyNetwork(nn.Module):
    ...
    def __init__(self, C, N, max_nodes, num_classes, criterion, search_space, affine=False, track_running_stats=True,
                 k=2, species='softmax', reg_type='l2', reg_scale=1e-3):
    ...
    def show_arch_parameters(self):
        with torch.no_grad():
            logging.info('arch-parameters :\n{:}'.format(
                process_step_matrix(self._arch_parameters, 'softmax', self._mask).cpu()))
    ...
    def genotype_100(self):
        genotypes = []
        alphas = process_step_matrix(self._arch_parameters, 'dirichlet', self._mask)
        for i in range(1, self.max_nodes):
            xlist = []
            for j in range(i):
                node_str = '{:}<-{:}'.format(i, j)
